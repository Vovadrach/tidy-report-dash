...
| 2026-07-05 | Fable 5 | **R-1 закрито:** Fixel самохост woff2 (Text-Regular/Bold + Display-SemiBold, 159 КБ, повна кирилиця; трохи понад орієнтир 120 КБ — прийнято, PWA кешує назавжди) |
| 2026-07-05 | Fable 5 | **R-2 закрито:** інваріант статусу — доменна функція `resolveStatus()` (без тригера БД) |
| 2026-07-05 | Fable 5 | **R-3 закрито:** pull-to-refresh — кастомний хук `usePullToRefresh` (touch events; нативний overscroll не працює в iOS standalone) |
| 2026-07-05 | Fable 5 | **R-4 закрито:** видалені date-fns, next-themes, react-hook-form + 30 інших (knip) |
| 2026-07-05 | Fable 5 | **R-5 закрито:** без діалогу — перша додана працівниця автоматично стає основною; записи без працівниць дозволені |
| 2026-07-05 | Fable 5 | **R-6 закрито:** вбудований `viewTransition` проп React Router 7 |
| 2026-07-05 | Fable 5 | **Відхилення:** агент не має доступу до продової БД → міграції v3 підготовлені (supabase/), застосування = дія користувача (стоп-крапка WP-1.4); до того фронт працює зі старою схемою через шар data/ одним вкладеним запитом, агрегати — domain/stats (контракт як у в'ю). Перемикання описане в src/data/README.md |
| 2026-07-05 | Fable 5 | **Відхилення:** темна тема означена токенами, але перемикача в UI немає (як у 1.x/2.x — застосунок світлий); увімкнення = клас `dark` на html, окремою задачею |
| 2026-07-05 | Fable 5 | **Виміри §1:** перший чанк 149.8 КБ gzip (<150 ✓); головна = 1 запит даних ✓; tsc/lint 0 помилок ✓; domain покритий 61 тестом ✓; smoke 6/6 ✓ |
